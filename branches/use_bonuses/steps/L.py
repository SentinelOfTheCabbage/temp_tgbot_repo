...
def on_call(menu=None, user=None, message=None):
    ...
    if user.bonuses:
        res_price = sum_price - int(user.bonuses)
        menu.database.update_bonuses(user.id, user.bonuses)
    else:
        res_price = sum_price
    ...
    if status == 'OK':
        if MKAD.is_mkad_zone(*coords):
            ...
            if sum_price >= 8000:
                ...
                A = delivery_cost - 0.06 * sum_price
                B = 0.6 * delivery_cost
                delivery_price = int(max(A, B))
                content_text += f'\nОбщая стоимость заказа c учётом доставки составит {res_price + delivery_price} руб. (доставка {delivery_price} руб.)'
    else:

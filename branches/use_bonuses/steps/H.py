...
def on_call(menu=None, user=None, message=None):
    ...
    evening_products = set(['pack_sibas23', 'pack_dorado34', 'prod_keta_stake'])
    basket_products = set([elem['key'] for elem in user.basket])
    if user.delivery_date == '22.02.21' and len(evening_products & basket_products) > 0:
        values = ['20-23']
        callbacks = ['20to23']
    elif user.delivery_date == datetime.now().strftime('%d.%m.%y'):
    ...
    keys = [e['key'] for e in user.basket]
    # if ('imper_oyster' in keys) or ('fish_forel' in keys and user.delivery_date == '26.06.20') :
    #     values = [' ','14-17','17-20','20-23']
    #     callbacks = ['Nan','14to17','17to20','20to23']

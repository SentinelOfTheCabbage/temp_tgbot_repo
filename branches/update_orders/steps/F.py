...
def on_call(menu=None, user=None, message=None):
    ...
    closest_date = None
    if ('fish_forel' in keys) or ('prod_ikra' in keys):
        # t = keys.index('fish_forel')
        # if menu.products['fish_forel']['in_stock'] > user.basket[t]['amount']:
        #     closest_date = '10.07.20'
        # else:
        #     closest_date = '10.07.20'
        closest_date = '24.07.20'
    ...
    if closest_date != None:
        DAY_DELTA = int((datetime.strptime(closest_date, '%d.%m.%y') -
                         (datetime.now() + timedelta(hours=3))).total_seconds() / 3600 / 24) + 1

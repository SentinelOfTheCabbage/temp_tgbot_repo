...
def get_restarts():
    ...
    restarts.append('заново')
    restarts.append('старт')
    restarts.append('рестарт')
    restarts.append('меню')
    restarts.append('начало')
    return restarts
...
def get_on_start(message):
    ...
    if '/start' in message.content:
        ...
        if 'referal' in message.content:
            kwargs['referal_id'] = message.content[15:]
        if 'predef' in message.content:
            kwargs['predef_prod'] = message.content[14:]
    return kwargs
if __name__ != '__main__':
    find_all_steps()
    os.chdir(getcwd)

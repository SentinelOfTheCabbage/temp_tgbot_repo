...
def on_return(menu=None, user=None, message=None):
    ...
    if 'prod' in product_key:
        ...
        user.basket[-1]['price'] = round(user.basket[-1]['amount']/user.basket[-1]['pack_size']*price)
    else:
        price = deepcopy(menu.products[product_key]['price'])
...
def on_call(menu=None, user=None, message=None):
    ...
    product_name = user.basket[-1]['description']
    ...
    for elem in enders:
        if elem in product_key:
            values = [str(round(el,3)) + enders[elem] for el in amounts]
...
def check_access(menu, user, message):
    if message.type == 'callback':
        pattern = '[0-9]{1,}'
        if re.fullmatch(pattern,message.content) != None:
